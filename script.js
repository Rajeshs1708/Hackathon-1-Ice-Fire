...
fm.style.borderRadius = '50px'
fm.style.width = '800px'
fm.style.height = '100px'
fm.style.boxShadow =
  '4px 4px 4px rgba(0,0,0,0.15),4px 4px 4px rgba(0,0,0,0.15),4px 4px 4px rgba(0,0,0,0.15)'

let label = document.createElement('label')
label.setAttribute('for', 'input')
label.innerHTML = "Type 'books' here : "
...
inp.setAttribute('placeholder', ' books...')
...
div.style.background = 'linear-gradient(to right, #42275a, #734b6d)'
...
let func = async value => {
  try {
    let url = await fetch(
      `https://www.anapioficeandfire.com/api/${value}?page=${page}&pageSize=${pageSize}`
    )
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      let li8 = document.createElement('br')
      let li9 = document.createElement('hr')
      let li10 = document.createElement('br')
      ...
      ol.append(li1, li2, li3, li4, li5, li6, li7, li8, li9, li10)
      div2.append(ol)
    }
  } catch (err) {
    console.log(err)
